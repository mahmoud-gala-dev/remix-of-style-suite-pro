...
- [~] Test coverage expanded: `rate-limit`, `csv`, `whatsapp`, `stripe-verify`, `otp-helpers`, `totp`, `booking-shift`, `recurring`, `cancel-policy`, and `reports-aggregate` (pure aggregation extracted from `getReportsSummary`). Full suite: 98 tests. Pure-extraction strategy now preferred over end-to-end server-fn mocking. (cycle #3)
