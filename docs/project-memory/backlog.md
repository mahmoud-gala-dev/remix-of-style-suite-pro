...
- [~] Test coverage expanded: `rate-limit`, `csv`, `whatsapp`, `stripe-verify`, `otp-helpers`, `totp`, `booking-shift`, `recurring`, `cancel-policy`, `reports-aggregate`, `format`, `client-ip`, `layout`, `csv-parse`, `push-vapid`, `theme`, `utils`, `translate-error` (i18n error-code mapping, EN/AR, Error unwrap, substring match, network_error fallback), and `error-page` (SSR fallback HTML shell). Suite: 174 tests. (cycle #3)
