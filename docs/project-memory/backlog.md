...
- [x] **Cycle #7 — Re-engagement cron** (`/api/public/cron/reengagement?inactiveDays=60&percent=15&validDays=30`): scans `customers` with prior `visits>0`, not blocked, and `last_visit < now()-inactiveDays`, mints a deterministic one-off coupon `REENG-<id8>-<yyyymm>` (idempotent per customer per month), and best-effort notifies via WhatsApp + email.
- [x] **Cycle #7 — Daily morning digest cron** (`/api/public/cron/daily-digest`): for each active branch, sends a best-effort WhatsApp summary to admin/reception staff with today's bookings count, yesterday's paid revenue, yesterday's no-shows, and current low-stock product count. No schema changes; cron-secret protected. Recommended cadence: once/day at branch opening time.
- [x] **Cycle #6 — Inventory low-stock alerts cron** (`/api/public/cron/inventory-low-stock?cooldownHours=24`): scans active `products` where `stock <= low_stock_threshold` and `last_low_stock_alert_at` is null or older than the cooldown, groups by branch, sends a best-effort WhatsApp digest to admin/reception staff in that branch, then stamps `last_low_stock_alert_at` so each product is alerted at most once per cooldown window. New `products.last_low_stock_alert_at` column.
- [x] **Cycle #6 — Birthday auto-coupon cron** (`/api/public/cron/birthday-coupons?percent=20&validDays=14`): scans `customers` whose birthday month/day matches today, mints a deterministic one-off coupon `BDAY-<id8>-<yyyy>` (idempotent — skipped if the code already exists), and sends best-effort WhatsApp/email greeting.
- [x] **Cycle #6 — Waitlist auto-promote cron** (`/api/public/cron/waitlist-promote?lookbackMin=30`): finds future bookings cancelled within the last N minutes, picks the oldest `waitlist.status='waiting'` entry per slot (same branch, matching service if specified, preferred_date ±1 day), notifies via WhatsApp, and stamps `notified_at` + `status='notified'` so each entry is promoted at most once.
