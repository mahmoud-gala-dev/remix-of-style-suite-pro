...
- [~] Test coverage expanded: `rate-limit`, `csv`, `whatsapp`, `stripe-verify`, `otp-helpers`, `totp`, `booking-shift`, `recurring`, and `cancel-policy` (terminal-status + 2h-notice rule extracted from `cancelBookingByToken`). Invoice server-fn integration tests still pending supabase mocking harness. (cycle #3)
