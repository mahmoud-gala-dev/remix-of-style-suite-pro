...
- [~] Test coverage expanded: `rate-limit`, `csv`, `whatsapp`, `stripe-verify`, `otp-helpers`, `totp`, `booking-shift`, and `recurring` (date expansion extracted from `createRecurringSeries`). Invoice server-fn integration tests still need a supabase mocking harness. (cycle #3)
